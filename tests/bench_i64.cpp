// GEB-ACE-002 Phase A — Pinned charged bench for int64_t
// Same pins as tests/bench.cpp (N, seed, sawtooth_mod, 3 trials).
// Build: g++ -O2 -std=c++20 -I public/geblomi-sort tests/bench_i64.cpp -o bench_i64 && ./bench_i64
...
constexpr int kSawtoothMod = 64;

std::vector<T> make_random(size_t n = kN) {
    std::mt19937 rng(kRandomSeed);
    std::uniform_int_distribution<T> dist(0, 1'000'000);
    std::vector<T> v(n);
    for (auto& x : v) x = dist(rng);
    return v;
}
std::vector<T> make_sorted(size_t n = kN) {
    std::vector<T> v(n);
    std::iota(v.begin(), v.end(), T{0});
    return v;
}
std::vector<T> make_reverse(size_t n = kN) {
    std::vector<T> v(n);
    for (size_t i = 0; i < n; ++i) v[i] = static_cast<T>(n - 1 - i);
    return v;
}
std::vector<T> make_patterned(size_t n = kN) {
    std::vector<T> v(n);
    size_t half = n / 2;
    for (size_t i = 0; i < half; ++i) v[i] = static_cast<T>(i);
    for (size_t i = half; i < n; ++i) v[i] = static_cast<T>(n - 1 - i);
    return v;
}
std::vector<T> make_sawtooth(size_t n = kN) {
    std::vector<T> v(n);
    for (size_t i = 0; i < n; ++i) v[i] = static_cast<T>(i % kSawtoothMod);
    return v;
}
...
Times measure_one(const std::vector<T>& base) {
    Times t;
...
int main() {
    ...
    std::cout << "GEB-ACE-002 Phase A int64 bench  N=" << kN
              << "  trials=" << kTrials << "  seed=" << kRandomSeed
    ...
    std::cout << "\nNOTE: losses/UNSTABLE reported, not CI-fail.\n";
    return 0;
}
